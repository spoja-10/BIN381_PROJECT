...
```
- Checked for duplicates using Indicator, SurveyYear, CharacteristicId, and Value → none found
...
- Columns like ByVariableLabel, RegionId, CILow, CIHigh, and LevelRank were dropped as they contained no usable information.

- DenominatorWeighted and DenominatorUnweighted were imputed using fill(direction = "downup").
# check data types
```{r}
data.frame(
  Column = names(lit_df),
...
```
- After winsorization, extreme values were capped, reducing the mean of Value to ~78.19 while retaining denominators as-is.
# save cleaned data
```{r}
